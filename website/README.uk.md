...
Єдине джерело перекладів знаходиться у корені репозиторію:
...
Перед `dev`, `test` і `build` скрипт `scripts/sync-locales.mjs` копіює їх у згенеровану директорію `public/locales/`. Не редагуйте згенеровану копію вручну.
