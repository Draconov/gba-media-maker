...
function isSupportedGBARune(character) { return Object.prototype.hasOwnProperty.call(GLYPHS, String(character || "").toUpperCase()); }
function normalizeGBAText(value) {
...
function sanitizeGBAText(value, maximum=0) {
...
function unsupportedGBARunes(value) { return normalizeGBAText(value).unsupported; }
function glyphLength(value) { return Array.from(String(value ?? "")).length; }
function glyphBits(character) { return GLYPHS[String(character || "").toUpperCase()] || 0; }
function encodeGBATextFixed(value, maximum) {
...
function safeGBAHeaderTitle(value) {
  let text="";
...
global.GBAText=Object.freeze({GBA_GLYPHS:GLYPHS,GBA_RUNTIME_CODES:RUNTIME_CODES,isSupportedGBARune,normalizeGBAText,sanitizeGBAText,unsupportedGBARunes,glyphLength,glyphBits,encodeGBATextFixed,safeGBAHeaderTitle});
})(window);
