...
static void native_overlay(const struct ClipDescriptor*c,u32 frame,int paused,int image,int show,const struct PlayerUI*ui){char tot[6];u32 total;int mode=show?ui->hud_mode:0;if((ui->hud_timer||ui->paused_ui)&&!image&&mode<2)mode=2;if(mode<=0){native_draw_audio_badges(c,ui);return;}if(image){rect3(VRAM0,0,132,240,28,0);text3n(VRAM0,8,138,c->title,12,0x7FFF);text3n(VRAM0,184,138,"IMAGE",5,0x03FF);return;}total=seconds_for_frame(c->frame_count?c->frame_count-1:0,c->vblanks_per_frame);time5(tot,total);if(mode==2){rect3(VRAM0,0,104,240,56,0);text3n(VRAM0,8,108,media_title(c),media_title_limit(c),0x7FFF);text3n(VRAM0,8,122,media_artist(c),media_artist_limit(c),0x5294);text3n(VRAM0,184,144,paused?"PAUSE":"PLAY",5,paused?0x03FF:0x03E0);}else rect3(VRAM0,0,140,240,20,0);native_audio_clock(c,frame,ui);text3n(VRAM0,52,144,"/",1,0x4210);text3n(VRAM0,64,144,tot,5,0x7FFF);native_draw_audio_badges(c,ui);}
...
static int play_media(const struct GlobalMetadata*m,const struct ClipDescriptor*c,u32 idx,struct PlayerUI*ui){ui->paused_ui=0;ui->start_pending=0;ui->select_pending=0;if(c->flags&CLIP_FLAG_MEDIA_IMAGE){ui->mute_timer=ui->volume_timer=0;return play_image(m,c,idx,ui);}if(c->flags&CLIP_FLAG_MEDIA_AUDIO){ui->hud_mode=2;ui->hud_last_visible=2;return play_audio(m,c,idx,ui);}ui->hud_mode=0;ui->hud_last_visible=2;if(!(c->flags&CLIP_FLAG_AUDIO))ui->mute_timer=ui->volume_timer=0;return play_video(m,c,idx,ui);}
