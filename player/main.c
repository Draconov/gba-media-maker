...
static void native_overlay(const struct ClipDescriptor*c,u32 frame,int paused,int image,int show,const struct PlayerUI*ui){char tot[6];u32 total;int mode=show?(ui?ui->hud_mode:2):0;if(ui&&(ui->hud_timer||ui->paused_ui)&&!image&&mode<2)mode=2;if(mode<=0){if(ui&&ui->mute_timer)mute_badge3(VRAM0,ui->muted);if(ui&&ui->volume_timer)volume_badge3(VRAM0,ui->volume_level);if(ui&&ui->seek_timer&&ui->seek_direction)seek_badge3(VRAM0,ui->seek_direction,c->seek_seconds?c->seek_seconds:5u);return;}if(image){rect3(VRAM0,0,132,240,28,0);text3n(VRAM0,8,138,c->title,12,0x7FFF);text3n(VRAM0,184,138,"IMAGE",5,0x03FF);return;}total=seconds_for_frame(c->frame_count?c->frame_count-1:0,c->vblanks_per_frame);time5(tot,total);if(mode==2){rect3(VRAM0,0,104,240,56,0);text3n(VRAM0,8,108,media_title(c),media_title_limit(c),0x7FFF);text3n(VRAM0,8,122,media_artist(c),media_artist_limit(c),0x5294);text3n(VRAM0,184,144,paused?"PAUSE":"PLAY",5,paused?0x03FF:0x03E0);}else rect3(VRAM0,0,140,240,20,0);native_audio_clock(c,frame,ui);text3n(VRAM0,52,144,"/",1,0x4210);text3n(VRAM0,64,144,tot,5,0x7FFF);if(ui&&ui->mute_timer)mute_badge3(VRAM0,ui->muted);if(ui&&ui->volume_timer)volume_badge3(VRAM0,ui->volume_level);if(ui&&ui->seek_timer&&ui->seek_direction)seek_badge3(VRAM0,ui->seek_direction,c->seek_seconds?c->seek_seconds:5u);}
...
static int play_audio(const struct GlobalMetadata*m,const struct ClipDescriptor*c,u32 idx,struct PlayerUI*ui){
 ...
 for(;;){u16 now,p;u32 elapsed,abs,target,sec;int ui_changed,action;wait_vblank();audio_service();ui_changed=tick_ui_timers(ui);now=keys_down();p=now&~prev;prev=now;elapsed=playback_timer_read();abs=base_sample+elapsed;if(ui_changed)native_refresh_audio_ui(c,f,paused,ui->hud_mode>0,ui);
  ...
  action=common_combo_action(now,p,m->clip_count>1,1,AUDIO_HUD_HOLD_VBLANKS,ui);
  ...
  action=held_seek_action(now,p,0,ui);if(action==ACTION_SEEK_BACK||action==ACTION_SEEK_FORWARD){u32 amount=step;int forward=(action==ACTION_FRAME_FORWARD||action==ACTION_SEEK_FORWARD);target=seek_target(f,c->frame_count,amount,forward);if(target!=f){if(action==ACTION_SEEK_BACK||action==ACTION_SEEK_FORWARD)start_audio_seek_feedback(ui,forward?1:-1);f=target;base_sample=seek_value(c,f);paused_sample=base_sample;audio_start_at(c,base_sample,paused,ui);playback_timer_reset();if(paused)playback_timer_pause();native_refresh_audio_ui(c,f,paused,ui->hud_mode>0,ui);lastsec=seconds_for_frame(f,c->vblanks_per_frame);save_position(m,idx,f);}continue;}
  if(p&KEY_UP){if(ui->volume_level<2)ui->volume_level++;ui->volume_timer=AUDIO_VOLUME_HOLD_VBLANKS;audio_apply_state(ui);native_refresh_audio_ui(c,f,paused,ui->hud_mode>0,ui);}
  if(p&KEY_DOWN){if(ui->volume_level>0)ui->volume_level--;ui->volume_timer=AUDIO_VOLUME_HOLD_VBLANKS;audio_apply_state(ui);native_refresh_audio_ui(c,f,paused,ui->hud_mode>0,ui);}
 }
}
