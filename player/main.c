...
static void seek_arrow4(volatile u16*d,u32 x,u32 y,int right){u32 r,k;for(r=0;r<7;r++)for(k=0;k<7;k++)if(seek_arrow_rows[r]&(1u<<k))p4(d,x+(right?6u-k:k),y+r,UI_YELLOW);}
static void seek_badge4(volatile u16*d,int dir,u32 sec){char n[2];u32 digits=sec>=10u?2u:1u,nw=digits==2u?7u:3u,bw=7u+2u+nw+4u,bx=(120u-bw)/2u,cx=bx+2u;if(digits==2u){n[0]=(char)('0'+udiv(sec,10u)%10u);n[1]=(char)('0'+sec%10u);}else n[0]=(char)('0'+sec%10u);rect4(d,bx,32,bw,10,UI_BLACK);if(dir<0){seek_arrow4(d,cx,33,0);text4n(d,cx+9u,34,n,digits,UI_WHITE);}else{text4n(d,cx,34,n,digits,UI_WHITE);seek_arrow4(d,cx+nw+2u,33,1);}}
