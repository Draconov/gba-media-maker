...
struct PlayerUI { int muted, volume_level, hud_mode, hud_last_visible,paused_ui; u16 hud_timer,mute_timer,volume_timer,seek_timer; int seek_direction,seek_hold_direction; u16 seek_hold_counter; int help_combo_latched,hud_combo_latched,clip_combo_latched,pause_button_latched,start_pending,select_pending; int shoulder_pending_direction; u16 shoulder_pending_counter; };
...
static void seek_badge4(volatile u16*d,int dir,u32 sec){u32 x=50;rect4(d,47,32,26,9,UI_BLACK);if(dir<0){char4(d,50,34,'<',UI_YELLOW);x=58;}if(sec>=10u){char4(d,x,34,(u8)('0'+udiv(sec,10u)%10u),UI_WHITE);char4(d,x+4,34,(u8)('0'+sec%10u),UI_WHITE);x+=8;}else{char4(d,x,34,(u8)('0'+sec),UI_WHITE);x+=4;}if(dir>0)char4(d,x+4,34,'>',UI_YELLOW);}
static void draw_video_hud(volatile u16*d,u32 f,const struct ClipDescriptor*c,const struct PlayerUI*ui){char a[6],b[6],fr[6];u32 total,cur,w;int mode=ui->hud_mode;if((ui->hud_timer||ui->paused_ui)&&mode<2)mode=2;if(mode<=0)return;cur=seconds_for_frame(f,c->vblanks_per_frame);total=seconds_for_frame(c->frame_count?c->frame_count-1:0,c->vblanks_per_frame);time5(a,cur);time5(b,total);rect4(d,0,68,120,mode==1?8:12,UI_BLACK);text4n(d,3,69,a,5,UI_WHITE);text4(d,24,69,"/",UI_DARK);text4n(d,29,69,b,5,UI_WHITE);if(mode==1)return;frame6(fr,f+1u);text4n(d,51,69,fr,6,UI_WHITE);rect4(d,4,77,112,2,UI_DARK);w=c->frame_count>1?udiv(f*112u,c->frame_count-1u):112;if(w>112u)w=112u;if(w)rect4(d,4,77,w,2,UI_YELLOW);}
static void render_frame_with_ui(const u8*p,u32 f,volatile u16*d,const struct ClipDescriptor*c,const struct PlayerUI*ui){render_pixels(p,d);draw_video_hud(d,f,c,ui);if(ui->mute_timer)mute_badge4(d,ui->muted);if(ui->volume_timer)volume_badge4(d,ui->volume_level);if(ui->seek_timer&&ui->seek_direction)seek_badge4(d,ui->seek_direction,c->seek_seconds?c->seek_seconds:5u);}
...
static void seek_badge3(volatile u16*d,int dir,u32 sec){u32 x=100;rect3(d,94,64,52,20,0);if(dir<0){char3(d,100,68,'<',0x03FF);x=116;}if(sec>=10u){char3(d,x,68,(u8)('0'+udiv(sec,10u)%10u),0x7FFF);char3(d,x+8,68,(u8)('0'+sec%10u),0x7FFF);x+=16;}else{char3(d,x,68,(u8)('0'+sec),0x7FFF);x+=8;}if(dir>0)char3(d,x+8,68,'>',0x03FF);}
...
static int play_audio(const struct GlobalMetadata*m,const struct ClipDescriptor*c,u32 idx,struct PlayerUI*ui){
 ...
 for(;;){u16 now,p;u32 elapsed,abs,target;int ui_changed,action;wait_vblank();audio_service();ui_changed=tick_ui_timers(ui);now=keys_down();p=now&~prev;prev=now;elapsed=playback_timer_read();abs=base_sample+elapsed;if(ui_changed)show_native_art(c,f,paused,0,ui->hud_mode>0,ui);
  ...
  if(p&KEY_A){paused=!paused;ui->paused_ui=paused;if(paused){paused_sample=abs;playback_timer_pause();audio_pause();}else{base_sample=paused_sample;audio_start_at(c,base_sample,0,ui);playback_timer_reset();}show_native_art(c,f,paused,0,ui->hud_mode>0,ui);}
...
static int play_video(const struct GlobalMetadata*meta,const struct ClipDescriptor*clip,u32 clip_index,struct PlayerUI*ui){
 ...
 for(;;){
  if(state!=PLAYBACK_RUNNING){
   ...
   if(action==ACTION_HELP){show_help_screen(&displayed_page,is_menu_mode(meta),meta->clip_count>1);render_and_show(current,frame,&displayed_page,clip,ui);next_frame_valid=0;previous_keys=keys_down();continue;}
  ...
  }
  {
   ...
   if(action==ACTION_PREV_CLIP){playback_timer_stop();audio_stop();save_position(meta,clip_index,frame);return PLAY_RESULT_PREV_CLIP;}
   if(action==ACTION_NEXT_CLIP){playback_timer_stop();audio_stop();save_position(meta,clip_index,frame);return PLAY_RESULT_NEXT_CLIP_DIRECT;}
   if(action==ACTION_HELP){playback_timer_pause();if(has_audio)audio_pause();show_help_screen(&displayed_page,is_menu_mode(meta),meta->clip_count>1);render_and_show(current,frame,&displayed_page,clip,ui);next_frame_valid=0;playback_clock_init(&clock,clip->vblanks_per_frame);playback_timer_reset();playback_clock_advance(&clock);if(has_audio)audio_start_for_frame(clip,frame,0,ui);previous_keys=keys_down();continue;}
...
static int play_media(const struct GlobalMetadata*m,const struct ClipDescriptor*c,u32 idx,struct PlayerUI*ui){ui->paused_ui=0;if(c->flags&CLIP_FLAG_MEDIA_IMAGE)return play_image(m,c,idx,ui);if(c->flags&CLIP_FLAG_MEDIA_AUDIO){ui->hud_mode=2;ui->hud_last_visible=2;return play_audio(m,c,idx,ui);}return play_video(m,c,idx,ui);}
