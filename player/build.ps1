...
$common = @("--target=arm-none-eabi", "-mcpu=arm7tdmi", "-marm", "-ffreestanding", "-fno-builtin", "-fno-stack-protector", "-fomit-frame-pointer", "-Oz", "-mllvm", "-enable-machine-outliner", "-mllvm", "-machine-outliner-reruns=3", "-Wall", "-Wextra")
...
& clang @common -c (Join-Path $PSScriptRoot "compiler_compat.S") -o (Join-Path $build "compiler_compat.o")
& ld.lld -T (Join-Path $PSScriptRoot "linker.ld") --gc-sections -o (Join-Path $build "player.elf") (Join-Path $build "startup.o") (Join-Path $build "main.o") (Join-Path $build "metadata.o") (Join-Path $build "compiler_compat.o")
...
try { $file.SetLength(32768) } finally { $file.Dispose() }
